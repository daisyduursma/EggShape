...
NT$lat<-round(NT$Lat_degrees + (NT$Lat_minutes/60),3)
NT$long<-round(NT$Long_degrees + (NT$Long_minutes/60),3)
...
SA$lat<-round(SA$lat_hour + (SA$lat_min/60),3)
SA$long<-round(SA$long_hour + (SA$long_min/60),3)
...
QU$lat<-round(QU$lat_deg + (QU$lat_min/60) + (QU$lat_sec/3600) *-1,3)
QU$long<-round(QU$long_deg + (QU$long_min/60)+ (QU$Long_sec/3600) *-1,3)
...
ausmus$lat<-round(ausmus$lat,3)
ausmus$lon<-round(ausmus$lon,3)
...
NVic$lon<-round(NVic$lon,3)
NVic$lat<-round(NVic$lat,3)
...
ANWC$lon<-round(ANWC$lon,3)
ANWC$lat<-round(ANWC$lat,3)
...
WAeggs$lon<-round(WAeggs$lon,3)
WAeggs$lat<-round(WAeggs$lat,3)
...
NRS$lon<-round(NRS$lon,3)
NRS$lat<-round(NRS$lat,3)
...
colnames(QV)<-c( 'Scientific.Name','lat','lon')
QV$lon<-round(QV$lon,3)
QV$lat<-round(QV$lat,3)
...
tmag$lon<-round(tmag$lon,3)
tmag$lat<-round(tmag$lat,3)
...
eggdat$lat<-round(eggdat$lat, digits = 3)
eggdat$lon<-round(eggdat$lon, digits = 3)
...
abbbs<-abbbs[,c("SCIENTIFIC_NAME","LAT","LON")]
abbbs$lat<-round(abbbs$LAT,3)
abbbs$lon<-round(abbbs$LON,3)
...
aves2$latitude<-round(aves2$latitude, digits = 3)
aves2$longitude<-round(aves2$longitude, digits = 3)
aves2<- na.omit(aves2[!duplicated( aves2),])
colnames(aves2)<-c( 'Scientific.Name','lat','lon')
aves2$Breeding<-0
aves2<- aves2[!duplicated(aves2),]
...
ebird$lat<-round(ebird$LATITUDE,3)
ebird$lon<-round(ebird$LONGITUDE,3)
...
ebird$lat<-round(ebird$lat,3)
ebird$lon<-round(ebird$lon,3)
...
gbif$lat<-round(gbif$decimallatitude,3)
gbif$lon<-round(gbif$decimallongitude,3)
...
eggdat$lat<-round(eggdat$lat,3)
eggdat$lon<-round(eggdat$lon,3)
eggdat<- eggdat[!duplicated(eggdat),]
